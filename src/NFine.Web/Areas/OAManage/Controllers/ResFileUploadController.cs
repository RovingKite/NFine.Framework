...
{
    [Area("OAManage")]
    public class ResFileUploadController : Controller
    {
        private readonly IHostingEnvironment _hostingEnvironment;

        public ResFileUploadController(IHostingEnvironment hostingEnvironment)
        {
        ...
        private void UploadFile()
        {
            ...
            string remsg = new NFine.Support.Upload().FileSaveAs(byteData, fileName, _isThumb, _isWater);
            ...
            if (status == "1")
            {
                ...
                string fileType = dic["type"].ToString();
                SaveResFile(fileName, fileType, filePath, thumbPath, fileSize, fileExt);
        ...
        public void EditorUploadImage()
        {
            ...
            //开始上传
            string remsg = new NFine.Support.Upload().FileSaveAs(byteData, fileName, _isThumbnail, _iswater);
            Dictionary<string, object> dic = JsonHelper.DataRowFromJSON(remsg);
            string status = dic["status"].ToString();
            string msg = dic["msg"].ToString();
            if (status == "1")
        ...
        #region 保存附件信息方法
        public void SaveResFile(string fileName, string fileType, string filePath, string thumbPath, string fileSize, string fileExt)
        {
            ResFileService resFileService = new ResFileService();
            ResFileInputDto resFileInputDto = new ResFileInputDto();
            resFileInputDto.FileName = fileName;
            resFileInputDto.FileType = fileType;
            resFileInputDto.FilePath = filePath;
            resFileInputDto.FileSize = Convert.ToInt64(fileSize);
            resFileInputDto.FileExt = fileExt;
            resFileInputDto.ThumbPath = thumbPath;
            resFileService.SubmitForm(resFileInputDto, "");
        }
        #endregion
