...
namespace NFineCore.Web.Areas.WeixinManage.Controllers
{
    ...
    public class WxMaterialController : Controller
    {
        ...
        public WxMaterialController(IHostingEnvironment hostingEnvironment)
        {
        ...
        #region Bootstrap FileInput 上传请求处理===================================
        public void UploadForeverMaterial()
        {
            ...
            if (status == "1")
            {
                ...
                string wxAppId = WxOperatorProvider.Provider.GetCurrent().AppId;
                AccessTokenResult accessTokenResult = AccessTokenContainer.GetAccessTokenResult(wxAppId);
                UploadForeverMediaResult uploadForeverMediaResult = MediaApi.UploadForeverMedia(accessTokenResult.access_token, webRootPath + filePath);
                if (uploadForeverMediaResult.ErrorCodeValue == 0)
                {
                    switch (type)
                    {
                        case "image": //保存至微信图片素材表
                ...
                {
                    showError(uploadForeverMediaResult.errmsg);
                }
            }
            else
            {
        ...
        #region 保存文件信息方法
        public void SaveAttach(string fileName, string filePath, string thumbPath, string fileSize, string fileExt)
        {
